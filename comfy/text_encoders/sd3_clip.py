...
class SD3ClipModel(mindspore.nn.Cell):
    ...
    def encode_token_weights(self, token_weight_pairs):
        ...
        if len(token_weight_pairs_g) > 0 or len(token_weight_pairs_l) > 0:
            ...
            if lg_out is not None:
                lg_out = mint.nn.functional.pad(lg_out, (0, 4096 - lg_out.shape[-1]))
                out = lg_out
            pooled = mint.cat((l_pooled, g_pooled), dim=-1)
        ...
        if out is None:
            out = mint.zeros((1, 77, 4096))

        if pooled is None:
            pooled = mint.zeros((1, 768 + 1280))
