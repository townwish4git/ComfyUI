...
def convert_tensor(extra, dtype, device=None):
    if hasattr(extra, "dtype"):
        if extra.dtype != mindspore.int and extra.dtype != mindspore.long:
            extra = comfy.model_management.cast_to_device(extra, None, dtype)
        else:
            extra = comfy.model_management.cast_to_device(extra, None, None)
    return extra
...
class BaseModel(mindspore.nn.Cell):
    ...
    def _apply_model(self, x, t, c_concat=None, c_crossattn=None, control=None, transformer_options={}, **kwargs):
        ...
        if c_concat is not None:
            xc = mint.cat([xc] + [comfy.model_management.cast_to_device(c_concat, None, xc.dtype)], dim=1)
        ...
        if context is not None:
            context = comfy.model_management.cast_to_device(context, None, dtype)
        ...
        for o in kwargs:
            ...
            if hasattr(extra, "dtype"):
                extra = convert_tensor(extra, dtype, None)
            elif isinstance(extra, list):
                ex = []
                for ext in extra:
                    ex.append(convert_tensor(ext, dtype, None))
                extra = ex
            extra_conds[o] = extra
        ...
        if len(model_output) > 1 and not mindspore.is_tensor(model_output):
            ...
            if concat_latent_image.shape[1:] != noise.shape[1:]:
                ...
                if noise.ndim == 5:
                    if concat_latent_image.shape[-3] < noise.shape[-3]:
                        concat_latent_image = mint.functional.pad(concat_latent_image, (0, 0, 0, 0, 0, noise.shape[-3] - concat_latent_image.shape[-3]), "constant", 0)
                    else:
            ...
            for ck in self.concat_keys:
                if denoise_mask is not None:
                    if ck == "mask":
                        ...
                        cond_concat.append(concat_latent_image)  # NOTE: the latent_image should be masked by the mask in pixel space
                    elif ck == "mask_inverted":
                        cond_concat.append(1.0 - denoise_mask)
                else:
                    if ck == "mask":
                        cond_concat.append(mint.ones_like(noise)[:, :1])
                    elif ck == "masked_image":
                        cond_concat.append(self.blank_inpaint_image_like(noise))
                    elif ck == "mask_inverted":
                        cond_concat.append(mint.zeros_like(noise)[:, :1])
                if ck == "concat_image":
                    if concat_latent_image is not None:
                        cond_concat.append(concat_latent_image)
                    else:
                        cond_concat.append(mint.zeros_like(noise))
            data = mint.cat(cond_concat, dim=1)
            return data
        return None
    ...
    def state_dict_for_saving(self, clip_state_dict=None, vae_state_dict=None, clip_vision_state_dict=None):
        ...
        if self.model_config.scaled_fp8 is not None:
            unet_state_dict["scaled_fp8"] = mindspore.tensor([], dtype=self.model_config.scaled_fp8)
        ...
        if self.model_type == ModelType.V_PREDICTION:
            unet_state_dict["v_pred"] = mindspore.tensor([])
    ...
    def set_inpaint(self):
        ...
        def blank_inpaint_image_like(latent_image):
            blank_image = mint.ones_like(latent_image)
    ...
    def memory_required(self, input_shape, cond_shapes={}):
        ...
        if comfy.model_management.xformers_enabled() or comfy.model_management.mindspore_attention_flash_attention():
            dtype = self.get_dtype()
            if self.manual_cast_dtype is not None:
                dtype = self.manual_cast_dtype
...
def unclip_adm(unclip_conditioning, device, noise_augmentor, noise_augment_merge=0.0, seed=None):
    ...
    for unclip_cond in unclip_conditioning:
        for adm_cond in unclip_cond["clip_vision_output"].image_embeds:
            ...
            c_adm, noise_level_emb = noise_augmentor(adm_cond, noise_level=mindspore.tensor([noise_level]), seed=seed)
            adm_out = mint.cat((c_adm, noise_level_emb), 1) * weight
            weights.append(weight)
            noise_aug.append(noise_augment)
            adm_inputs.append(adm_out)

    if len(noise_aug) > 1:
        adm_out = mint.stack(adm_inputs).sum(0)
        noise_augment = noise_augment_merge
        noise_level = round((noise_augmentor.max_noise_level - 1) * noise_augment)
        c_adm, noise_level_emb = noise_augmentor(adm_out[:, :noise_augmentor.time_embed.dim], noise_level=mindspore.tensor([noise_level]))
        adm_out = mint.cat((c_adm, noise_level_emb), 1)

    return adm_out

class SD21UNCLIP(BaseModel):
    def __init__(self, model_config, noise_aug_config, model_type=ModelType.V_PREDICTION, device=None):
        super().__init__(model_config, model_type, device=None)
    ...
    def encode_adm(self, **kwargs):
        ...
        if unclip_conditioning is None:
            return mint.zeros((1, self.adm_channels))
        else:
...
class SDXLRefiner(BaseModel):
    ...
    def encode_adm(self, **kwargs):
        ...
        out = []
        out.append(self.embedder(mindspore.tensor([height])))
        out.append(self.embedder(mindspore.tensor([width])))
        out.append(self.embedder(mindspore.tensor([crop_h])))
        out.append(self.embedder(mindspore.tensor([crop_w])))
        out.append(self.embedder(mindspore.tensor([aesthetic_score])))
...
#         out = []
#         out.append(self.embedder(mindspore.tensor([fps_id])))
#         out.append(self.embedder(mindspore.tensor([motion_bucket_id])))
#         out.append(self.embedder(mindspore.tensor([augmentation])))

#         flat = mint.flatten(mint.cat(out)).unsqueeze(dim=0)
#         return flat
...
#             latent_image = mint.zeros_like(noise)
...
#         out = []
#         out.append(self.embedder(mint.flatten(mindspore.tensor([augmentation]))))
...
#         out = []
#         out.append(self.embedder(mint.flatten(mindspore.tensor([augmentation]))))
#         out.append(self.embedder_512(mint.deg2rad(mint.fmod(mint.flatten(90 - mindspore.tensor([elevation])), 360.0))))
#         out.append(self.embedder_512(mint.deg2rad(mint.fmod(mint.flatten(mindspore.tensor([azimuth])), 360.0))))

#         out = list(map(lambda a: utils.resize_to_batch_size(a, noise.shape[0]), out))
#         return mint.cat(out, dim=1)
...
#         out['c_crossattn'] = comfy.conds.CONDCrossAttn(cross_attn)
#         device = kwargs["device"]
#         task_emb = mindspore.tensor([1, 0]).float().to(device)
#         task_emb = mint.cat([mint.sin(task_emb), mint.cos(task_emb)]).unsqueeze(0)
...
#             clip_img = mint.cat(embeds, dim=1)
#         else:
#             clip_img = mint.zeros((1, 1, 768))
#         out["clip_img"] = comfy.conds.CONDRegular(clip_img)
#         out["sca"] = comfy.conds.CONDRegular(mint.zeros((1,)))
#         out["crp"] = comfy.conds.CONDRegular(mint.zeros((1,)))
...
#         global_embed = mint.cat([seconds_start_embed, seconds_total_embed], dim=-1).reshape((1, -1))
#         out['global_embed'] = comfy.conds.CONDRegular(global_embed)

#         cross_attn = kwargs.get("cross_attn", None)
#         if cross_attn is not None:
#             cross_attn = mint.cat([cross_attn.to(device), seconds_start_embed.repeat((cross_attn.shape[0], 1, 1)), seconds_total_embed.repeat((cross_attn.shape[0], 1, 1))], dim=1)
#             out['c_crossattn'] = comfy.conds.CONDRegular(cross_attn)
#         return out
...
#         out['image_meta_size'] = comfy.conds.CONDRegular(mindspore.Tensor([[height, width, target_height, target_width, 0, 0]]))
#         return out
...
#             out["c_size"] = comfy.conds.CONDRegular(mindspore.Tensor([[height, width]]))
#             out["c_ar"] = comfy.conds.CONDRegular(mindspore.Tensor([[kwargs.get("aspect_ratio", height/width)]]))
...
#         if attention_mask is not None:
#             out['attention_mask'] = comfy.conds.CONDRegular(attention_mask)
#             out['num_tokens'] = comfy.conds.CONDConstant(max(1, mint.sum(attention_mask).item()))
#         cross_attn = kwargs.get("cross_attn", None)
#         if cross_attn is not None:
#             out['c_crossattn'] = comfy.conds.CONDRegular(cross_attn)
#         return out
...
#             out['guiding_frame_index'] = comfy.conds.CONDRegular(mindspore.Tensor([guiding_frame_index]))
...
#         latent_image = self.model_sampling.calculate_input(mindspore.tensor([sigma_noise_augmentation], device=latent_image.device, dtype=latent_image.dtype), latent_image)
...
#         attention_mask = kwargs.get("attention_mask", None)
#         if attention_mask is not None:
#             if mindspore.ops.numel(attention_mask) != attention_mask.sum():
#                 out['attention_mask'] = comfy.conds.CONDRegular(attention_mask)
#             out['num_tokens'] = comfy.conds.CONDConstant(max(1, mint.sum(attention_mask).item()))
#         cross_attn = kwargs.get("cross_attn", None)
#         if cross_attn is not None:
...
#         out['speaker_embeds'] = comfy.conds.CONDRegular(mint.zeros(noise.shape[0], 512, device=noise.device, dtype=noise.dtype))
...
#                 generator = mindspore.Generator(device="cpu")
#                 generator.manual_seed(kwargs.get("seed", 0) - 10)
#                 noise = mint.randn(image.shape, generator=generator, dtype=image.dtype, device="cpu").to(image.device)
