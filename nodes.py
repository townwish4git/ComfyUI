...
class VAELoader:
    ...
    def load_taesd(name):
        ...
        if name == "taesd":
            sd["vae_scale"] = mindspore.tensor(0.18215)
            sd["vae_shift"] = mindspore.tensor(0.0)
        elif name == "taesdxl":
            sd["vae_scale"] = mindspore.tensor(0.13025)
            sd["vae_shift"] = mindspore.tensor(0.0)
        elif name == "taesd3":
            sd["vae_scale"] = mindspore.tensor(1.5305)
            sd["vae_shift"] = mindspore.tensor(0.0609)
        elif name == "taef1":
            sd["vae_scale"] = mindspore.tensor(0.3611)
            sd["vae_shift"] = mindspore.tensor(0.1159)
        return sd
    ...
    def load_vae(self, vae_name):
        if vae_name == "pixel_space":
            sd = {}
            sd["pixel_space_vae"] = mindspore.tensor(1.0)
...
class EmptyLatentImage:
    ...
    def generate(self, width, height, batch_size=1):
        latent = mint.zeros([batch_size, 4, height // 8, width // 8])
